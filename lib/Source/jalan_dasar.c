...
void normalSwap0(uint16_t time) {
	setServoIK(&right.front, normalXY, normalXY + offsetY, normalZ + 0.5, time);
	setServoIK(&right.rear, normalXY, -normalXY + offsetY, normalZ - 1, time);
	setServoIK(&left.mid, normalXMid, offsetY, normalZ, time);
}

void normalSwap1(uint16_t time) {
	setServoIK(&left.front, normalXY, normalXY + offsetY, normalZ - 0.5, time);
	setServoIK(&left.rear, normalXY, -normalXY + offsetY, normalZ, time);
	setServoIK(&right.mid, normalXMid, offsetY, normalZ - 1, time);
}
