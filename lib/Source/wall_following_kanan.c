...
void kanan(uint16_t time, colorStatus color) {
	uint8_t right, left, correct = 0, front, colorCorrect = 0;
	char srf[4][4];
	colorMatch = 0;
	timer5Start(color);
	wf = 1;
...
void putarKanan90(uint16_t time){
	...
	for(a = 0; a < 2; a++) {
		moveRobot(0, 0, 33, time);
		moveRobot(0, 0, 33, time);
	}
	moveRobot(0, 0, 10, time);
	moveNormal(time);
	wf = temp;
}
...
uint8_t kananProx(uint16_t time, colorStatus color) {
	uint8_t right, left, correct = 0, front;
	char srf[16][4];
	colorMatch = 0;
	timer5Start(color);
	wf = 1;
