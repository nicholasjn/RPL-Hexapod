...
public class Main_Activity extends Activity implements SensorEventListener {
	...
	//Variable declaration for setting up the bluetooth
	...
	//Store the UUID of Bluetooth device
    public static final UUID MY_UUID = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");

	//Same as #define in C/C++
	...
	//Thread for send and receive data via Bluetooth
	ConnectedThread nThread;

	//Variable for Accelerometer Sensor
	...
	Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case SUCCESS_CONNECT:
				...
				status = true;
				String sByte = "#NJN#";
				connectedThread.write(sByte.getBytes());
				Log.i(tag, "connected");
				break;
			case MESSAGE_READ:
...
//	@Override
	...
	private void init() {
		...
		receiver = new BroadcastReceiver(){
			...
			if(BluetoothDevice.ACTION_FOUND.equals(action)){
				...
				// run some code
				Toast.makeText(getApplicationContext(), "Discovering Bluetooth Devices", 0).show();
	...
	private class ConnectThread extends Thread {
		...
		public void run() {
			...
			mBtSocket = mmSocket;
			nThread = new ConnectedThread(mmSocket);
